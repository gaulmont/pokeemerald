...
static const u16 sSplitIconsPal[] = INCBIN_U16("graphics/custom/physicability_icons.gbapal");
static const u8 sSplitIconsTiles[] = INCBIN_U8("graphics/custom/physicability_icons.4bpp");
...
static u8 ShowSplitIcon(u8 split)
{
    ...
    if (sMonSummaryScreen->splitIconSpriteId == 0xFF)
        sMonSummaryScreen->splitIconSpriteId = CreateSprite(&sSpriteTemplate_SplitIcons, 216, 104, 0);
...
static void PrintMoveDetails(u16 move)
{
    ...
    if (move != MOVE_NONE)
    {
        if (sMonSummaryScreen->currPageIndex == PSS_MODE_BOX)
        {
            if (move != 0)
            {
                if (IS_MOVE_STATUS(gBattleMoves[move]))
                    ShowSplitIcon(2);
                else if (IS_MOVE_SPECIAL(gBattleMoves[move]))
                    ShowSplitIcon(1);
                else
                    ShowSplitIcon(0);
            }
