    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        ...
        else
        {
            ...
            else // hp goes down
            {
                ...
                if (IS_MOVE_PHYSICAL(gBattleMoves[gCurrentMove]) && !(gHitMarker & HITMARKER_x100000) && gCurrentMove != MOVE_PAIN_SPLIT)
                {
                ...
                else if (!IS_MOVE_PHYSICAL(gBattleMoves[gCurrentMove]) && !(gHitMarker & HITMARKER_x100000))
                {
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_RECOIL_33: // Double Edge
                ...
                gBattlescriptCurrInstr = sMoveEffectBS_Ptrs[gBattleCommunication[MOVE_EFFECT_BYTE]];
                break;
            case MOVE_EFFECT_RECOIL_50: // Custom Head Smash
...
static void atk86_stockpiletobasedamage(void)
{
    ...
    else
    {
        ...
        {
            gBattleMoveDamage = CalculateBaseDamage(&gBattleMons[gBattlerAttacker], &gBattleMons[gBattlerTarget], gCurrentMove,
                                                    gSideStatuses[GET_BATTLER_SIDE(gBattlerTarget)], 100,
